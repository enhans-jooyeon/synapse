...
- **Glass material reworked into a two-tint, Apple-style system.** Glass now applies to the **CommandPalette and AppLauncher** only; the **Composer follow-up panel is now a solid `bg.raised` panel** (it's small, dense, and sits over thread text, where glass reads muddy). The overlay glass now has **two tints chosen by the surface behind it** — dark glass over light/neutral surfaces (light theme), light glass over dark/colorful (dark theme) — because glass only reads as glass when its tint *contrasts* the backdrop (light-on-light / dark-on-dark go opaque). Text is `fg.inverse`, which retires the old 0.82 opacity floor. New tokens: `glass.surface` (inverted, `.56`/`.55`), `glass.blur` (20→26), `glass.filter` (adds a saturate/brightness lift so the backdrop glows through), `glass.border`, `glass.accent` (brightened AI accent on dark glass), and `shadow.glass` (the layered contact+ambient drop plus the tint-mirrored rim — bright top rim + inner ring on dark, soft neutral hairline on light). **SY015 widened** to allow `backdrop-filter: var(--sy-glass-filter)` (was blur-only). foundations §5 rewritten (selection rule + B anatomy); the three component specs + `preview.html` updated. Reduced-transparency falls back opaque (`bg.inverse`/`bg.raised`). **Two legibility fixes on the palette:** all glass text is `fg.inverse` (the search input was rendering black), and row hover/selection now uses a new `glass.selected` token (a subtle translucent overlay, ~10% white on dark / ~6% ink on light) instead of the opaque `bg.selected`, which read as a jarring light bar on dark glass.
