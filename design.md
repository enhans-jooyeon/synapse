...
**Version 2.7.0 · Enhans · This file is the entry point. Read it before generating any UI.**
...
7. Max one `primary` button and one Banner per region (the Composer counts as its own region — its send is that region's one primary). **The accent marks AI capability, not the send button (reallocated 2026-08-03).** `Composer send` is **`primary`/black** — it is the most predictable control on the screen and does not need an accent to be found. The `brand` accent (azure `#0073E6`) is spent instead on **AI capability**: the CommandPalette's "Ask agent" escape hatch, and the Composer's capability toggles (ontology/grounding on), where the state changes what the agent can do and is genuinely easy to miss. Max one brand accent per region either way. Operational agent actions (Run/Retry/Resume) remain `primary`, which now makes send consistent with them rather than an exception. Operational agent actions (Run/Retry/Resume) stay `primary`/black: "executes an agent" never earns the point color.
