...
- **Harness-anatomy roadmap (June's framing: system prompt · context · tools · memory · subagents).** System prompt = design.md + process docs ✅. **Context** = `product-context.md` ✅ (v1). **Tools** = `tools/synapse.py` CLI ✅ (lookup/validate/gate/list, wired into the cycle). **Remaining: Memory** (wire the refinement register + AI-gap decision register as read-at-generation input) and **Subagents** (structure the cycle phases — research/intake/review — as delegated subagents, wrapping the design-plugin skills). Tools was chosen as a CLI (wrap-able as an MCP server later) so it runs in any bash-capable agent tool.
