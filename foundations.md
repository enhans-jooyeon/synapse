...
**Glass material (two tints, jurisdiction corrected):** glass belongs where LIVE CONTENT sits behind the panel — blur over a scrim samples an already-darkened backdrop and reads muddy gray by construction (the scrim-gating had the physics backwards; maintainer caught it on the Template Library). Jurisdiction: **unscrimmed floating layers only — CommandPalette (which drops its scrim; the frost itself is the focus device, esc/click-away still dismiss) and the AppLauncher.** The **Composer follow-up panel is NOT glass — it is a solid `bg.raised` panel** (`border.default` hairline + `shadow.overlay`): it is small, dense, and sits directly over thread text, where glass reads muddy on light and a heavy slab on dark, so it stays opaque for legibility. Scrimmed blocking layers (Modal, Drawer) are OPAQUE `bg.raised` — the scrim already de-emphasizes. Menus, popovers, tooltips, toasts stay opaque — blur costs compound during scroll/pointer interaction; new glass surfaces require a governance proposal.

**Selection rule — the tint inverts the surface.** Glass reads as glass only when its tint contrasts what's behind it; light-on-light and dark-on-dark go opaque. So the tint is chosen by surface, which maps to theme: **light theme → DARK glass; dark theme → LIGHT glass.** Both are the same material, one token set (`glass.*`) whose mode values flip. Text is always `fg.inverse` (white on dark glass, ink on light glass) → legible regardless of the backdrop, which retires the old 0.82 opacity floor.

**Anatomy:** `glass.surface` fill (dark `rgba(12,13,18,.56)` / light `rgba(255,255,255,.55)`) + `backdrop-filter: var(--sy-glass-filter)` (blur 26 + a saturate/brightness lift so the backdrop glows through) + `glass.border` hairline + `shadow.glass`. The shadow is a layered **contact + ambient** drop plus the edge treatment, and the edge treatment MIRRORS by tint: **dark glass** gets a bright top rim + inner light-edge ring (white insets read against the dark fill); **light glass** gets a soft neutral hairline and a whisper-soft rim — its definition comes from the shadow, not a white outline (white-on-light halos). An optional top-left sheen (a faint white radial) completes the specular read. **All text on glass is `fg.inverse` — the search input included** (a common miss: the input stays ink and reads black on dark glass). **Row hover/selection uses `glass.selected`** (a subtle translucent overlay: ~10% white on dark glass, ~6% ink on light) — never `bg.selected`, whose light fill reads as a jarring opaque bar on dark glass. `prefers-reduced-transparency: reduce` falls back opaque (`bg.inverse` for dark glass, `bg.raised` for light). `backdrop-filter` is legal ONLY as `var(--sy-glass-filter)` or `blur(var(--sy-glass-blur))` (SY015). The AI/accent affordance on glass uses `glass.accent` (brightened `#8AA0FF` on dark glass; `#0621C4` on light).
