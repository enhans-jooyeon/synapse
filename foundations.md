...
**Glass material (two tints, jurisdiction corrected):** glass belongs where LIVE CONTENT sits behind the panel — blur over a scrim samples an already-darkened backdrop and reads muddy gray by construction (the scrim-gating had the physics backwards; maintainer caught it on the Template Library). Jurisdiction: **unscrimmed floating layers only — CommandPalette (which drops its scrim; the frost itself is the focus device, esc/click-away still dismiss), the AppLauncher, and the Composer's follow-up panel.** Scrimmed blocking layers (Modal, Drawer) are OPAQUE `bg.raised` — the scrim already de-emphasizes. Menus, popovers, tooltips, toasts stay opaque — blur costs compound during scroll/pointer interaction; new glass surfaces require a governance proposal.
