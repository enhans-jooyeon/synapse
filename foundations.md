...
- **Stacking:** floating/pinned elements take exactly one `--sy-z-*` token — `sticky` (100, table headers, toolbars, solid Banner strip) < `dropdown` (200, menus/popovers/palette) < `drawer` (300) < `modal` (400) < `toast` (500) < `tooltip` (600). Arbitrary z-index values are forbidden; if two things fight, the fix is the scale, not a 9999. **Local stacking is a separate, sanctioned vocabulary (ratified 2026-08-05):** sibling ordering *inside* one component takes small integer z-index (−1 to 2) and MUST sit inside an isolated stacking context — `isolation: isolate` on the component root (or an equivalent context) — so local values structurally cannot compete with the page's layer scale. The two vocabularies split by element class, like the radius tier: ordering against page-level chrome → a `--sy-z-*` token; ordering siblings within a component → ≤ 2 + isolation. A local value above 2 means the component is doing layer work and belongs on the scale. Gate: SY023.
...
- Single icon family, stroke-based, 1.5px stroke at 20px grid (Lucide — down-weighted from Lucide's 2px default to 1.5). The usable set is the **closed concept→icon registry in `icons.md`** — unlisted concepts get no icon; unlisted Tabler names are violations.
- Sizes: **12px** (metadata scale — status glyphs inside Badges, source-type marks in provenance rows, and any icon paired with 11–12px text), 16px (inline, compact controls), 20px (default controls, navigation), 24px (empty states, feature moments). **No other sizes** — 10, 11, 14 and 18 are off-scale. **24px is also the ceiling of the icon tier: stroke art rendered above 24px is an *illustration*, governed by §8.1, and is outside the `icons.md` registry's closed-set rule** (ruling 2026-08-06 — the registry exists to make one concept render as one glyph in UI chrome; a 32–64px mark is compositional artwork, not chrome).
