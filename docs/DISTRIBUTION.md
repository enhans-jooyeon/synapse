...
| Component library `@enhans-jooyeon/synapse` (React + tokens) | The product codebase | Versioned **npm package** | **Seed only** — 13 of 68 components; not published |
...
1. **An installable `@enhans-jooyeon/synapse`** — the `storybook/` workspace is `private: true` and implements 13 of 68 components (Button · Badge · Input · Card + the 2026-08-03 chat set). You cannot ship a design system that implements 13 of 68 components. Until the library is built out, authors have little to import and the "component provenance" gate (§6) has nothing to check against.
