...
**Footer REMOVED — the tray is the whole composer (2026-08-03, June).** The tray holds one row: **circular leading `+` · textarea · mic · circular send**. There is no footer, above or below. The tray is a **pill (`radius.full`)** at one text row *with no draft-owned content*, and relaxes to **`lg` (16)** once the text wraps **or any attachment, chip or quote is present** — a pill-topped attachment region reads wrong at that width.
