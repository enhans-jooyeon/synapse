...
**Sizes:** `xs` (height-xs 24, label 11, **padding-x 8**, icon 12) · `sm` (height-sm 32, label 13/12, **padding-x 10**) · `md` (height-md 36, default, **padding-x 12**) · `lg` (height-lg 40, heroes only, **padding-x 16**). **Radius by size** (not one fixed value): `xs` → `radius.xs` (4) · `sm` → `radius.sm` (8) · `md` → `radius.control-md` (10, default) · `lg` → `radius.md` (12); the pill option (`primary` + `lg` + Guided heroes/empty-state first-use, `default` or `brand` target only) overrides to `full`.
