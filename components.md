...
| `brand` | `action.brand-bg` fill | **The POINT / brand color — graphite, achromatic and mode-inverting (`#1A1A1F` light / `#F2F2F4` dark; variant renamed accent→brand).** Jurisdiction: brand-identity objects + conversational-AI CTAs — Ask agent / Composer send, max 1/screen. Operational agent actions (Run/Retry/Resume) stay `primary`/black — executing a configured run never earns the point color; only conversationally invoking the agent does. The label inverts with the fill (`action.brand-fg`: white in light, near-black in dark) and clears AA in both modes. |
...
**Anatomy (filled):** label (required, `--sy-label-size` medium, above) · field (height-md, `bg.sunken` fill, borderless, radius `10`; the `sm`/`lg` field sizes step to `sm`/`md` per foundations §5) · optional helper text (caption, `fg.secondary`) · error text (caption, `status.danger`, replaces helper).
...
**`compact` variant:** for small containers (Drawer sections, popovers, NotificationCenter, HoverCard bodies): no illustration, single `body-sm` `fg.secondary` line + optional inline Link action, padding 16. Full-size EmptyStates inside small overlays are forbidden — use compact.
...
**Anatomy:** rows of term + description. Two layouts: `side-by-side` (term `label` `fg.secondary` left column, min-width 120px sized to the widest term of the active locale, max 200px; description `body` right) and `stacked` (term above description; use in narrow panes <360px). Rows separated by `--sy-space-2`; optional full-bleed `border.subtle` row dividers.
