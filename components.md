...
**Anatomy (shared, `horizontal` — the default):** 28px height · padding `0 space-3` · `bg.raised-2` fill · 1px `border.overlay` hairline · radius `full` · `shadow.lg` · `z.dropdown` · `label-sm` type (sans 12/18 medium — never a raw font-size, `foundations.md` §2.2) · standard entrance. Multiple actions are separated by a 1px `border.default` hairline, 12px tall. Icons are 12px, from the registry.
**`rail` variant:** the same fill, hairline and radius rotated vertical — `shadow.xs` (it sits *on* content rather than floating above a scroll region, so it earns less lift), 4px padding, icon-only children, hover-reveal. One rail per anchor.
...
**Variants (closed):** `horizontal` · `rail`.
**Consumers (closed — extension is governance):** `SelectionPill` (§18), `Thread`'s "Jump to latest", `ResponseToolbar` `media` (§21/§35).
