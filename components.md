...
**Footer REMOVED — the tray is the whole composer (2026-08-03, June).** The tray holds one row: **circular leading `+` · textarea · mic · circular send**. There is no footer, above or below. The tray is a **pill (`radius.full`)** only while its content occupies **a single line**; past that it is a rounded rectangle at **`lg` (16)** with **the same radius on all four corners** — the rect never takes asymmetric corners. It also takes the rect when **any attachment, chip or quote is present**, regardless of line count — a pill-topped attachment region reads wrong at that width.
...
2. **§24's refine-prompt has no trigger.** The `edit` ghost icon-button at the input's top-right was removed. The whole §24 affordance — the closed preset menu, the reviewable rewrite, the Undo Toast — is unreachable until it gets a new entry point. **Open.**
