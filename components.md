...
Split follows its main button's variant (`primary` or `secondary`; `accent` is the point AI-CTA and is not used in split buttons); the chevron segment is 28px wide, same height/variant, separated by a 1px divider in the border color (or `alpha.white-16` on solid fills).
