...
**Footer below the tray + shape morph — ADOPTED 2026-08-03.** The tray holds the input row only: **circular leading `+` · textarea · mic · circular send**. The agent picker, capability toggle and model selector sit in a **footer row below the container**, on the page. The tray is a **pill (`radius.full`) at one row** and relaxes to **`lg` (16)** once the text wraps.

**The two were one decision.** The pill only reads as a pill because the footer left the tray — with the controls inside, a single-line composer is still two rows (~80px) and `radius.full` on that reads as a lozenge. Adopting either alone would have been wrong.
...
**This amends `foundations.md` §5**, which assigned the Composer tray a flat `md`. The tray's radius is now conditional, and it is the only container in the system whose radius depends on its content.
