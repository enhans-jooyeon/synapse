...
**Optical nudge on the input text (2026-08-03).** The 22px line box centres geometrically in the 36px row, but Pretendard's cap-height centre sits above its line-box centre, so the text reads ~1px high. The textarea takes `transform: translateY(1px)` — the numeral-nudge family the system already uses (ResponseToolbar's thumbs take `translateX(∓1px)`, Badge `md` takes a 1px top nudge). **`transform` is deliberate over padding or margin:** it moves the glyphs without changing `scrollHeight`, so the morph's one-row baseline and wrap detection are untouched. Do not replace it with padding.
