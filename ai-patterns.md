...
| `running` | Spinner (`loader-circle`) | `text.secondary` | thread · run · batch |
| `awaiting-input` | `user`, `status.warning` | `text.secondary` + trailing action | thread · run |
| `partial` | `triangle-alert`, `status.warning` | `text.secondary` + count link | thread · run · batch |
...
| `cancelled` | `square`, `border.strong` | `text.tertiary` | thread · run · batch |
...
- **Marker form:** circular 18px chips with bare numerals on the emphasis tint — a citation is quiet emphasis; brackets and mono are retired (they read as code, not annotation).
...
**Quote highlight:** the selected passage takes a **bare `ai.surface` fill** while quoted — radius `xs`, no outline, `box-decoration-break: clone` so each wrapped line gets its own tint box. *Amended 2026-08-03: this specified an inset `ai.border` hairline, which made the highlight read as a Badge rather than a marker — tint + outline is the formula `components.md · ProposalCard` names as forbidden (the wireframe callout), and every other highlight in the system is a bare tint (§22 flash, §23 slots, §24 rewrite mark, §31 field highlight). It stays `ai.*` rather than `emphasis.*`: §1 bars emphasis tokens from AI surfaces.*
**ComposerQuote:** choosing Reply inserts a quote bar into the Composer above the textarea — `ai.surface` fill, `ai.fg` text, reply icon, single-line ellipsis, × remove. Radius `sm` — the nested-element tier (`foundations.md` §5). Concentricity is **exempt** here: the tray's 12px inset equals its `md` radius, and §5 exempts elements inset ≥ the outer radius, so the quote bar clears the corner region entirely. Max one quote per send; quoting replaces any existing quote. The sent message renders the quote above the user text.
...
## 26. Voice input (maintainer decision; supersedes the earlier "parked" ruling)
...
- Starters MUST model a complete, well-formed prompt in the workspace's domain — never "무엇이든 물어보세요" (a starter that doesn't teach prompt structure is decoration). **Label ≠ insertion (demo-verified):** the chip label is a short task handle (2–4 words); selecting it inserts the full exemplar prompt (scope, format, constraints spelled out). The label is a handle; the insertion is the lesson. Chip-honesty (§19) is preserved because starters insert for editing — they never send.
...
## 35. Feedback capture (completes ResponseToolbar)
