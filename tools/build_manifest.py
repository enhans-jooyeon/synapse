...
C = {
 "Button": {"purpose": "Trigger an action; never navigation (that is Link).",
  "variants": ["primary", "secondary (tonal)", "ghost", "danger", "accent (AI actions only)"],
  "sizes": ["sm", "md", "lg"],
  "key_rules": ["max 1 primary per region", "accent = agent-invoking actions only", "text-only labels by default; icon+text only for accent AI actions and toolbar/filter contexts (approved icons)", "pill radius only in Guided heroes / empty-state first-use", "loading keeps width; disabled is a fill, never opacity", "danger labels semibold; danger hover darkens to AA"]},
 ...
 "Input (text)": {"purpose": "Single-line text entry (text/email/password/number/search).",
  "key_rules": ["filled anatomy: bg.sunken, borderless; focus rings and opens to bg.page", "label above, always; placeholder is example content only", "errors name the fix (border.error)", "mixed bulk values show 'Mixed'/'여러 값'", "no fixed widths on translatable content"]},
 ...
 "Badge": {"purpose": "Static annotation: status, counts, categories. Never interactive (that is Chip).",
  ...
  "key_rules": ["status vocabulary = content.md §3.3 closed set", "solid has named jobs only: urgent marks (1 solid color/view), ops-table opt-in, neutral=release markers (must expire), ai=live-activity beacon (disappears on completion)", "dot uses status.*-bg-solid fills", "with-icon option: 12px registry status icon for triple redundancy (subtle/solid only)", "md optical nudge: line-height 1 + 1px top padding (md only)"]},
 "Chip": {"purpose": "Interactive compact element: select, refine, remove, accept a suggestion. Clickable=Chip, informs-only=Badge.",
  "variants": ["input (removable)", "filter (toggleable)", "category (click-to-filter, system-assigned viz color)", "suggestion (ai surface; Console/empty states only, max 3)"],
 ...
 "Card": {"purpose": "Bounded group of related content.",
  "variants": ["flat (default: bg.surface, borderless)", "outlined (separable/interactive objects)", "elevated (max 1/page; sole static shadow)", "ai (agent content)", "stat (KPI anatomy)"],
 ...
 "Sidebar (app navigation)": {"purpose": "The single global navigation surface.",
  "key_rules": ["240px expanded/64 rail", "container padding 12; items 32px, 4px gaps; group labels 16px top pad", "labels never truncate when expanded — shorten the label", "max 2 nesting levels"]},
 ...
 "Modal": {"purpose": "Blocking decision or focused short task.",
  "key_rules": ["widths 400(confirm)/480/640 max", "footer: Cancel + one primary (danger for destructive, consequences named by count+noun)", "no modal-on-modal; z.modal"]},
 "Drawer": {"purpose": "Side detail/edit without leaving context; right side only; 480/640.",
  "key_rules": ["inherits opener density (full-screen data drawers may be dense)", "wide 800 variant for data review (DiffView)", "z.drawer"]},
 "Popover / Menu": {"purpose": "Anchored floating panel; menus of actions/options.",
  "key_rules": ["border.overlay (none in light, visible in dark) + shadow.overlay; z.dropdown", "items 32px with 4px gaps; destructive last; dividers full-bleed", "search row optional >8 items", "one submenu level max"]},
 ...
 "Toast": {"purpose": "Transient outcome notification; bottom-right; max 3; z.toast.",
  "key_rules": ["same-scheme surface (raised-2 + border.overlay + shadow)", "errors 8s + manual dismiss; never for validation or decisions", "undo convention: reversible-lite ops get Undo toast (8s) instead of Popconfirm — never both"]},
 "Banner / Alert": {"purpose": "Persistent inline notice for a page/section.",
  "variants": ["colors: neutral,info,success,warning,danger", "emphasis: subtle, solid (app-critical strip; max 1 in app; semibold text)"],
 ...
 "EmptyState": {"purpose": "Every list/table/search has one. Flavors: first-use, no-results, error (+retry).",
  "key_rules": ["always an action when possible", "no illustrations in v1", "explanations balance-wrapped", "compact variant (one line, no icon) required inside small overlays"]},
 ...
 "CommandPalette": {"purpose": "⌘K universal entry: navigation, actions, ask-agent.",
  "key_rules": ["never dead-ends — final row is 'Ask agent: {query}'", "grouped results, kbd hints, match highlighting", "z.dropdown"]},
 ...
 "Composer": {"purpose": "The message/instruction input for Console and ask-agent surfaces.",
  "key_rules": ["never disabled during generation (empty-send is the one sanctioned disable)", "send↔stop morph in place", "Enter sends, Shift+Enter breaks, Enter during IME composition NEVER sends", "attachments as input Chips; agent/scope picker ghost", "drafts persist; no formatting toolbar; one per screen"]},
 "ResponseToolbar": {"purpose": "Actions on an agent message: copy · regenerate · thumbs · overflow.",
  "key_rules": ["agent messages only; fixed order", "hover-reveal default, persistent in dense consoles", "regenerate on latest message only", "thumbs selected = stroke + bg.selected circle, never filled icons", "no destructive actions"]},
 "AgentStep": {"purpose": "One row of visible agent work (step/tool call).",
  "key_rules": ["closed states: pending/running/success/failed(+Retry)/skipped", "collapses to '5 steps · 12s' summary on completion", "tool ids in mono; payloads collapsed; max 1 nesting level", "role=log aria-live=polite"]},
 ...
 "ProposalCard": {"purpose": "Human-in-the-loop approval of a consequential agent action.",
  "key_rules": ["ai surface; Approve=accent (danger if destructive), Reject=secondary", "no auto-approve, no countdowns", "resolved collapses to attribution row, never deleted", "diffs: tint backgrounds + gutter markers, never color alone"]},
 ...
 "SegmentedControl": {"purpose": "Exclusive 2–5-way switch, immediate effect (not Tabs, not Radio).",
  "key_rules": ["content-based widths; disabled whole-control only"]},
 ...
 "FileUpload": {"purpose": "Dropzone or button; per-file progress rows.",
  "key_rules": ["constraints visible before selection; violations named per file", "no combined progress bar"]},
 "SplitPanel": {"purpose": "Resizable workbench panes; the divider is the sanctioned mixed-density boundary.",
  "key_rules": ["≤3 panes; min widths 280/200; ratio persists; not in focus archetypes"]},
...
def main():
    ...
    manifest = {
        ...
        "never": ["raw color/spacing/radius/type values (semantic tokens + typography styles only)",
                  ...
                  ">1 primary button or Banner per region; accent fill on non-AI actions",
