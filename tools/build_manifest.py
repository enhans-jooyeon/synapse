...
C = {
 "Button": {"purpose": "Trigger an action; never navigation (that is Link).",
  "variants": ["primary", "secondary (tonal)", "ghost", "danger", "accent (POINT color — graphite, mode-inverting — conversational-AI CTA only: Ask agent / Composer send)"],
  "sizes": ["sm", "md", "lg"],
  "key_rules": ["max 1 primary per region", "conversational-AI entry (Ask agent / Composer send) = accent = the graphite point color (max 1/screen); operational agent actions (Run/Retry/Resume) stay primary/black — executing a configured run never earns the point color; slate stays the AI surface, the graphite point is a distinct accent on top (never accent-on-slate)", "text-only labels by default; icon+text only for accent AI actions and toolbar/filter contexts (approved icons)", "pill radius only in Guided heroes / empty-state first-use", "icon-only buttons are always square (width = size height)", "optical trim: icon+text buttons drop 2px padding on the icon side", "loading keeps width; disabled is a fill, never opacity", "danger labels semibold; danger hover darkens to AA"]},
 ...
 "Input (text)": {"purpose": "Single-line text entry (text/email/password/number/search).",
  "key_rules": ["filled anatomy: bg.sunken, borderless; focus = 1px NEUTRAL border.focus-input + fill opens to bg.page (no offset ring on fields; non-entry controls keep the blue 2px offset ring)", "label above, always; placeholder is example content only", "errors name the fix (border.error)", "mixed bulk values show 'Mixed'/'여러 값'", "no fixed widths on translatable content", "affixes: one leading registry icon + one trailing unit-suffix or registry icon, inside the fill, non-focusable", "add-ons (InputGroup): fused leading/trailing text, leading/trailing Select, trailing secondary Button, ⌘K kbd hint, or help-circle tooltip beside the label — one per side, shared outer shape (border.default seams), never a dropdown+button on one side, never decorative"]},
 ...
 "AppLauncher": {"purpose": "Browsable overlay tile grid of system + user apps (the Application entry); distinct from CommandPalette's ⌘K search.",
  "key_rules": ["centered glass-scrimless overlay (like CommandPalette; reduced-transparency → opaque bg.raised; z.modal), radius lg", "optional search + system-apps grid + your-apps grid under micro-label headers", "tiles = outlined Card (icon medallion + name + caption), whole-tile click opens, hover-lift", "keyboard traversal + ↵ opens; empty groups hide on filter", "no decorative tile color, no nested modal, fixed responsive column steps"]},
 "Divider": {"purpose": "Separate content with a 1px rule — the standalone borders-first divider.",
  ...
  "key_rules": ["1px only — border.subtle inside components, border.default for stronger boundaries + the vertical variant", "labeled = rule · caption/micro label (fg.tertiary) · rule", "margins come from surrounding rhythm, never baked in", "reach for it only when a visible line aids grouping — whitespace/component borders first", "no thick or colored dividers; one divider weight per region", "role=separator (aria-orientation=vertical for vertical); a decorative rule is aria-hidden"]},
 "ToggleButton": {"purpose": "A pressable on/off control — pin, favorite, mute, panel/display toggle.",
  ...
  "sizes": ["sm", "md", "lg"],
  "key_rules": ["distinct from Switch (instant setting) and SegmentedControl (exclusive choice)", "on-state = bg.selected + fg.primary; icon stays stroke — favorite star is the sole fill-on-active exception", "aria-pressed reflects state; icon-only requires aria-label", "never the point/accent color for the on-state — selection is neutral bg.selected", "group is multi-select (role=group + label); exclusive choice is SegmentedControl", "icon-only toggles are square (width = control height)"]},
}
