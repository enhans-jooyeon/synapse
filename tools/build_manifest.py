...
C = {
 ...
 "Chip": {"purpose": "Interactive compact element: select, refine, remove, accept a suggestion. Clickable=Chip, informs-only=Badge.",
  "variants": ["input (removable)", "filter (toggleable)", "category (click-to-filter, system-assigned viz color)", "suggestion (borderless ai.surface fill, hover ai.surface-hover; Console/empty states only, max 3)"],
 ...
 "Card": {"purpose": "Bounded group of related content.",
  "variants": ["flat (default: bg.surface, borderless)", "outlined (separable/interactive objects)", "elevated (max 1/page; sole static shadow)", "ai (agent content)", "stat (KPI anatomy; emphasized opt-in = slate emphasis.* fill, max 1/grid, v6.11)"],
 ...
 "Modal": {"purpose": "Blocking decision or focused short task.",
  "key_rules": ["widths 400(confirm)/480/640 max", "glass panel (v6.22; reduced-transparency → opaque)", "footer: Cancel + one primary (danger for destructive, consequences named by count+noun)", "no modal-on-modal; z.modal"]},
 "Drawer": {"purpose": "Side detail/edit without leaving context; right side only; 480/640.",
  "key_rules": ["inherits opener density (full-screen data drawers may be dense)", "wide 800 variant for data review (DiffView)", "glass when scrimmed; no-scrim variant stays opaque (v6.22)", "z.drawer"]},
 ...
 "CommandPalette": {"purpose": "⌘K universal entry: navigation, actions, ask-agent.",
  "key_rules": ["never dead-ends — final row is 'Ask agent: {query}'", "glass material (v6.22; reduced-transparency → opaque)", "grouped results, kbd hints, match highlighting", "z.dropdown"]},
 ...
 "Composer": {"purpose": "The message/instruction input for Console and ask-agent surfaces.",
  "key_rules": ["never disabled during generation (empty-send is the one sanctioned disable)", "send↔stop morph in place", "ComposerQuote bar (v6.12): ai.surface radius-xs, max 1/send; follow-up panel above composer, max 4 rows, never auto-sends", "slash commands (v6.15): / scopes palette to agent actions; ghost completion accepted with →, never Tab; closed glossary only", "follow-up panel = glass material (v6.23, the one anchored glass exception)", "templates: / gains 템플릿 group + bookmark footer button opens the library (search, 내/팀 groups, 새 템플릿 만들기 footer — v6.29); slot chips = emphasis.surface, → moves slots, unfilled slot blocks send (v6.25 §23)", "authoring coach: max one non-blocking quality hint; pen-line opens the CLOSED refinement-preset menu (다듬기/자세히/간결/범위/형식 — v6.27), rewrite replaces draft w/ Undo, disabled on empty", "voice = dictation only (v6.26 §26): tray morphs to recording bar (cancel / pulsing danger dot + timer / pause / primary check confirm); transcript inserts at caret, NEVER auto-sends; no audio in thread", "agent-picker menu: search + grouped rows + one submenu + mandatory request-footer escape row", "Enter sends, Shift+Enter breaks, Enter during IME composition NEVER sends", "attachments as input Chips; agent/scope picker ghost", "drafts persist; no formatting toolbar; one per screen"]},
 ...
 "SourceChip": {"purpose": "Inline provenance marker [n] for agent claims; popover with source detail.",
  "key_rules": ["never fake citations — unsourced claims get the 'Model knowledge' badge", "≤3 per sentence"]},
