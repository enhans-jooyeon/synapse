...
C = {
 ...
 "AssistantPanel": {"purpose": "Persistent docked/floating global agent — compact chat that maximizes into the Console. Composite (Composer + message stream), like CommandPalette.",
  "key_rules": ["floating brand circular launcher (the shared circular exception) → docked panel (bg.raised, border.default, shadow.overlay, z.dropdown)", "header (squared Avatar 24 + name + maximize + close) · ai.surface message stream (AgentStep/ProposalCard/SourceChip) · one docked Composer", "reuses Composer/ResponseToolbar/AgentStep — never a re-implemented chat", "exactly one per app; non-modal (no scrim); hidden on the full Console page; never overlaps a primary action region", "no second persistent chat, no auto-open/nagging"]},
