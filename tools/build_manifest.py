...
C = {
 ...
 "Input (text)": {"purpose": "Single-line text entry (text/email/password/number/search).",
  "key_rules": ["filled anatomy: bg.sunken, borderless; focus = 1px NEUTRAL border.focus-input + fill opens to bg.page (v6.10; no offset ring on fields; non-entry controls keep the blue 2px offset ring)", "label above, always; placeholder is example content only", "errors name the fix (border.error)", "mixed bulk values show 'Mixed'/'여러 값'", "no fixed widths on translatable content"]},
 ...
 "Card": {"purpose": "Bounded group of related content.",
  "variants": ["flat (default: bg.surface, borderless)", "outlined (separable/interactive objects)", "elevated (max 1/page; sole static shadow)", "ai (agent content)", "stat (KPI anatomy; uniform — the v6.11 emphasized opt-in removed v6.40)"],
 ...
 "Table": {"purpose": "Workhorse of dense mode: records with aligned columns.",
  "key_rules": ["18 closed cell renderers fix alignment/format per column; empty cell = em dash", "header row: label-size medium fg.tertiary, transparent fill (v6.5), hairline rule only; sort glyph space reserved", "selection column: 40px, zero padding, centered both axes — control cell, not text cell (v6.24.1)", "emphasis column opt-in (v6.11): emphasis.surface fill marks current period/totals, max 1/table", "status columns default dot+text; solid opt-in for ops walls", "focus tables frameless (bare header); dense/scrolling tables framed", "column menu closed set; bulk selection bar; inline edit for text/number/select cells; 1-level grouping; virtualize >200 rows", "no zebra striping"]},
 ...
 "Modal": {"purpose": "Blocking decision or focused short task.",
  "key_rules": ["widths 400(confirm)/480/640 max", "opaque bg.raised (v6.31 — glass retired from scrimmed layers)", "footer: Cancel + one primary (danger for destructive, consequences named by count+noun)", "no modal-on-modal; z.modal"]},
 "Drawer": {"purpose": "Side detail/edit without leaving context; right side only; 480/640.",
  "key_rules": ["inherits opener density (full-screen data drawers may be dense)", "wide 800 variant for data review (DiffView)", "opaque bg.raised always (v6.31 — glass retired from scrimmed layers)", "z.drawer"]},
 "Popover / Menu": {"purpose": "Anchored floating panel; menus of actions/options.",
  "key_rules": ["border.overlay (none in light, visible in dark) + shadow.overlay; z.dropdown", "6px container padding (concentric: 10-6=4 item radius, v6.8); items 32px with 4px gaps; destructive last; dividers full-bleed — binds EVERY horizontal rule in any floating panel, incl. search/hint/footer rows (v6.21.2)", "search row optional >8 items", "one submenu level max"]},
 ...
 "Toast": {"purpose": "Transient outcome notification; bottom-right; max 3; z.toast.",
  "key_rules": ["same-scheme surface (raised-2 + border.overlay + shadow)", "errors 8s + manual dismiss; never for validation or decisions", "contents first-line aligned; dismiss × = compact 20px box (not a form control — v6.27.2)", "undo convention: reversible-lite ops get Undo toast (8s) instead of Popconfirm — never both"]},
 "Banner / Alert": {"purpose": "Persistent inline notice for a page/section.",
  "variants": ["colors: neutral,info,success,warning,danger", "emphasis: subtle (borderless status tint, no border/rail — v6.7.1), solid (app-critical strip; max 1 in app; semibold text)"],
 ...
 "EmptyState": {"purpose": "Every list/table/search has one. Flavors: first-use, no-results, error (+retry).",
  "key_rules": ["always an action when possible", "no illustrations in v1", "icon medallion: sunken circle + two concentric hairline rings (v6.4); error flavor tints it danger-bg", "explanations balance-wrapped", "compact variant (one line, no icon) required inside small overlays"]},
 ...
 "CommandPalette": {"purpose": "⌘K universal entry: navigation, actions, ask-agent.",
  "key_rules": ["never dead-ends — final row is 'Ask agent: {query}'", "glass material, SCRIMLESS (v6.31 — frost is the focus device; reduced-transparency → opaque)", "grouped results, kbd hints, match highlighting", "z.dropdown"]},
 ...
 "Composer": {"purpose": "The message/instruction input for Console and ask-agent surfaces.",
  "key_rules": ["never disabled during generation (empty-send is the one sanctioned disable)", "send↔stop morph in place", "ComposerQuote bar (v6.12): ai.surface radius-xs, max 1/send; follow-up panel above composer, max 4 rows, never auto-sends", "slash commands (v6.15): / scopes palette to agent actions; ghost completion accepted with →, never Tab; closed glossary only", "follow-up panel = glass material (v6.23, the one anchored glass exception)", "templates: / = expert quick-insert; bookmark opens the Template Library Modal (glass 640: search + grouped rich cards w/ description + cloze preview + 새 템플릿 만들기 footer — v6.30); slot chips = emphasis.surface, → moves slots, unfilled slot blocks send (§23)", "authoring coach: max one non-blocking quality hint; pen-line opens the CLOSED refinement-preset menu (다듬기/자세히/간결/범위/형식 — v6.27), rewrite replaces draft w/ Undo, disabled on empty", "voice = dictation only (v6.26 §26): tray morphs to recording bar (cancel / pulsing danger dot + timer / pause / primary check confirm); transcript inserts at caret, NEVER auto-sends; no audio in thread", "agent-picker menu: search + grouped rows + one submenu + mandatory request-footer escape (no model rows — reversed v6.42)", "model selector (v6.42, trailing v6.44): menu grouped by provider under micro-label headers, real product names in mono, 자동 row = agent default (v6.45); per-conversation, defaults from agent, lockable; never changes permissions/approval", "tools popover (plug, v6.41): per-conversation capability switches; disabled tool → agent asks; enabling never bypasses ProposalCard", "composer footer (v6.44): leading = + menu (첨부/템플릿/도구) + agent picker; trailing = model + mic (send-adjacent, never in + menu) + send; kbd hint removed; refine-prompt = contextual pen at input top-right, rendered only with a non-empty draft; 1 visible leading icon default, 5 hard cap", "input-pattern laws (v6.47): starters = zero-state only, insert never send, dismissible; chip label IS the query; selection pill = 답장/설명/재생성 closed set, thread append-only; attachment captions advisory-only; batch = per-item queue, one failure never aborts, ProposalCard still gates; ghost text → accepts never Tab, suppressed during Hangul composition", "Enter sends, Shift+Enter breaks, Enter during IME composition NEVER sends", "attachments as input Chips; agent/scope picker ghost", "drafts persist; no formatting toolbar; one per screen"]},
 "ResponseToolbar": {"purpose": "Actions on an agent message: copy · regenerate · thumbs · overflow.",
  "key_rules": ["agent messages only; fixed order", "hover-reveal default, persistent in dense consoles", "regenerate on latest message only — creates variant N+1, never destroys; answer header carries the ‹n/N› pager (v6.25 §22)", "thumbs selected = stroke + bg.selected circle, never filled icons", "no destructive actions", "media variant: vertical rail beside a MediaGroup ONLY when media is the sole message content (v6.17.7) — text+media replies use the message toolbar alone, never both"]},
 "AgentStep": {"purpose": "One row of visible agent work (step/tool call).",
  "key_rules": ["closed states: pending/running/success/failed(+Retry)/skipped", "collapses to '5 steps · 12s' summary on completion", "tool ids in mono; payloads collapsed; max 1 nesting level", "role=log aria-live=polite", "named working line above steps while generating (pulse, no shimmer); long replies open with title + duration badge + collapse (v6.13)"]},
 ...
 "SegmentedControl": {"purpose": "Exclusive 2–5-way switch, immediate effect (not Tabs, not Radio).",
  "key_rules": ["content-based widths; disabled whole-control only", "concentric geometry (v6.8): radius-sm container, 4px padding, radius-xs segments — inner = outer − inset; assembled control = 36 focus / 32 dense"]},
 ...
 "FileUpload": {"purpose": "Dropzone or button; per-file progress rows.",
  "key_rules": ["constraints visible before selection; violations named per file", "no combined progress bar", "dropzone: dashed border.strong + radius lg + medallion; drag-over = border.focus-input + emphasis.surface (v6.14); dashed = drop targets only"]},
 "SplitPanel": {"purpose": "Resizable workbench panes; the divider is the sanctioned mixed-density boundary.",
  "key_rules": ["≤3 panes; min widths 280/200; ratio persists; not in focus archetypes", "container = section shell: radius xl + hairline, flush panes (v6.9)"]},
 ...
 "Tree": {"purpose": "Hierarchy display/selection; folders, org units.",
  "key_rules": ["rows 28/24; indent 20/level; max 4 levels then drill-in", "checkbox mode uses mixed-state parents", "drop target = focus insertion line", "no guide lines; middle-out truncation + tooltip"]},
 ...
 "ContextMenu": {"purpose": "Right-click Menu on dense data surfaces.",
...
def build():
    ...
    manifest = {
        ...
        "archetypes": {"workbench": "dense", "object": "focus", "settings": "focus", "guided": "focus", "console": "focus"},
        "densities": ["focus", "dense"],
        "locales": ["en", "ko"],
